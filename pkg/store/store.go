...
func (e *ExpireMap) set(ctx context.Context, key, value string, ttl time.Duration) {
	e.lock.Lock()
	defer e.lock.Unlock()
	if key == "" {
		return
...
// Get 获取值，如果不存在或已过期返回 ""
func (e *ExpireMap) get(ctx context.Context, key string) string {
	e.lock.RLock()
...
// Del 删除 key
func (e *ExpireMap) del(ctx context.Context, key string) {
	e.lock.Lock()
	defer e.lock.Unlock()
	if key == "" {
		return
